...
static void mtdoops_add_reason(char *oops_buf, enum kmsg_dump_reason reason, enum mtdoops_log_type type, int index, int nextpage)
{
	...
	if (nextpage > 0)
		ret_len =  snprintf(str_buf, 200,
				...
				index, UTS_VERSION, kdump_reason[reason], ts.tm_year+1900, ts.tm_mon+1, ts.tm_mday, ts.tm_hour, ts.tm_min, ts.tm_sec, log_type[type]);
	else
		ret_len =  snprintf(str_buf, 200,
				"\n\n# Index: %d\t\n## Build: %s\t\n## Reason: %s\n### %04ld-%02d-%02d %02d:%02d:%02d\n#### Log type: %s\t\n```\t\n",
				index, UTS_VERSION, kdump_reason[reason], ts.tm_year+1900, ts.tm_mon+1, ts.tm_mday, ts.tm_hour, ts.tm_min, ts.tm_sec, log_type[type]);
...
static int mtdoops_add_pmsg_head(char *oops_buf, enum mtdoops_log_type type)
{
	char str_buf[40] = {0};
	int ret_len = 0;
			    ...
			    enum kmsg_dump_reason reason)
{
	...
	mtdoops_add_reason(cxt->oops_buf+buffer_start, reason, MTDOOPS_TYPE_DMESG, cxt->nextcount, cxt->nextpage);

	buffer_start += ret_len;
	buffer_start += mtdoops_add_pmsg_head(cxt->oops_buf + buffer_start, MTDOOPS_TYPE_PMSG);
	pmsg_addr = (void *)phys_to_virt((ramoops_data.mem_address + ramoops_data.mem_size) - ramoops_data.pmsg_size);
	pmsg_cpy_size = record_size - buffer_start;

	if(temp_pmsg_start >= pmsg_cpy_size)
		memcpy(cxt->oops_buf + buffer_start, pmsg_addr + (temp_pmsg_start - pmsg_cpy_size), pmsg_cpy_size);
	else {
		pmsg_rem = pmsg_cpy_size - temp_pmsg_start;
		memcpy(cxt->oops_buf + buffer_start, pmsg_addr + (ramoops_data.pmsg_size - pmsg_rem), pmsg_rem);
		memcpy(cxt->oops_buf + (buffer_start + pmsg_rem), pmsg_addr, temp_pmsg_start);
	}
